...
const Home = () => {
  ...
  const loadAvailableSlots = async (date, fieldId) => {
    ...
    try {
      ...
      const response = await axios.get(`/api/calendar/available/slots?date=${formattedDate}&field_id=${fieldId}`);
  ...
  return (
    <div className="container">
      <div className="card">
        ...
        <p>Wybierz boisko i datę, aby zobaczyć dostępne terminy i dokonać rezerwacji.</p>
      </div>
        ...
        <div className="card">
          ...
          >
            {fields.map(field => (
              <option key={field.id} value={field.id}>
                {field.name} - {field.field_type === 'full' ? 'Pełnowymiarowe' : 
                               field.field_type === 'half' ? 'Połowa boiska' : 
                               field.field_type === 'small' ? 'Małe boisko' : field.field_type}
                {field.surface_type && ` (${field.surface_type === 'artificial' ? 'Sztuczna trawa' : 
                                             field.surface_type === 'natural' ? 'Trawa naturalna' : 
                                             field.surface_type})`}
              </option>
            ))}
          </select>
          ...
          {selectedFieldData && (
            <div style={{ marginTop: '15px', padding: '15px', backgroundColor: '#f8f9fa', borderRadius: '5px' }}>
              ...
              {selectedFieldData.description && <p>{selectedFieldData.description}</p>}
              <p><strong>Maksymalna liczba graczy:</strong> {selectedFieldData.max_players}</p>
              {selectedFieldData.hourly_rate && (
                <p><strong>Cena za godzinę:</strong> {selectedFieldData.hourly_rate} PLN</p>
              )}
            </div>
          )}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
        ...
        <div className="card">
          <h2>Dostępne terminy na {format(selectedDate, 'd MMMM yyyy', { locale: pl })}</h2>
                  ...
                  <ul style={{ listStyle: 'none', padding: 0 }}>
                    {reservations.map(reservation => (
                      <li key={reservation.id} style={{ 
                        ...
                        <span className={`badge badge-${reservation.status}`} style={{ marginLeft: '10px' }}>
