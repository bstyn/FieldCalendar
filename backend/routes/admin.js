...
router.get('/fields', async (req, res) => {
  try {
    const result = await db.query('SELECT * FROM football_fields ORDER BY name ASC');
    res.json(result.rows);
  } catch (error) {
...
router.post('/fields',
  ...
  async (req, res) => {
    try {
      ...
      const { name, description, field_type, surface_type, max_players, hourly_rate } = req.body;
...
router.put('/fields/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const { name, description, field_type, surface_type, max_players, hourly_rate, is_active } = req.body;
    ...
    const result = await db.query(
      ...
      [name, description, field_type, surface_type, max_players, hourly_rate, is_active, id]
    );
...
router.delete('/fields/:id', async (req, res) => {
  try {
    ...
    const result = await db.query('DELETE FROM football_fields WHERE id = $1 RETURNING *', [id]);
...
router.post('/calendar',
  ...
  async (req, res) => {
    try {
      ...
      const { title, description, start_date, end_date, event_type, field_id } = req.body;

      const result = await db.query(
        `INSERT INTO calendar_events (title, description, start_date, end_date, event_type, field_id, created_by)
         VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
        [title, description, start_date, end_date, event_type || 'available', field_id, req.user.id]
      );
